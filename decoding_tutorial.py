...
data_path = 'C:/Users/raque/Dropbox/EEG_Support/Gardens/Workshop/Mengqiao_Raquel/Data/ds000117_pruned'
...
# as used above, pooled all of these distinct conditions:
    ...
    # The plots we made above show the number of trials averaged. You can see if
    # it checks out.
...
evoked_face_famous = epochs['face/famous'].average()                           
...
# Since the condition with the lowest count has 50 trials, we should end up 
# with 100 in total:
...
# We create the classifier:
    ...
    # of this tutorial.
    # The online solver “liblinear” is a type of optimization algorithm that updates 
    # its parameters incrementally as it processes each data point. This approach 
    # is often used in large-scale machine learning applications, where it is not 
    # feasible to process all the data at once due to memory or computational constraints.
    # More about this here: https://tinyurl.com/48sjkd43
...
# condition of each trial at each timepoint based on spatio-temporal data.
...
# data along the last dimension. # What is the last dimension of the data? Time:
X.shape
...
# characteristic curve (AUC-ROC). The ROC curve plots the True Positive Rate (TPR) 
# against the False Positive Rate (FPR) for different threshold values. 
...
# specificity (1 - False Positive Rate) across all possible classification thresholds. 
# Models with higher AUC-ROC are better at discriminating between classes.

# Let's score it, how many scores do you expect to get?
...
scores = cross_val_multiscore(time_gen, X, y, cv=5, n_jobs=None) #break code by leaving folds open
print(scores.shape)
...
coef_filt = get_coef(time_decod, "filters_", inverse_transform=True) #retrieve the filter weights
evoked_time_decod_filt = EvokedArray(coef_filt, epochs.info, tmin=epochs.times[0]) #constract the 
...
# To create a more interpretable situation, we can also plot the "spatial patterns".
...
# the backward model to a forward model, and then projecting the data through it.
...
# That's cool. Now try to plot the spatial filter and activity pattern at the 
# time-point where the decoding score is maximal
...
maxtimind = np.argmax(scores)
# Plot the topography of the activity patterns
evoked_time_decod_pat.plot_joint(
    times=epochs.times[maxtimind], title="patterns", **joint_kwargs
)   
# Plot the topography of the spatial filter
evoked_time_decod_filt.plot_joint(
    times=epochs.times[maxtimind], title="filters", **joint_kwargs
)   
